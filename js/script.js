...
const PARTICLE_SIZE = 2.5; // View heights
const SPEED = 2500; // Milliseconds
...
function createParticle(canvas) {
  const colour = {
    r: 255,
    g: randomNormal({ mean: 125, dev: 10 }),
    b: 50,
    a: rand(0, 5),
  };
  return {
    x: -2,
    y: -2,
    diameter: Math.max(0, randomNormal({ mean: PARTICLE_SIZE, dev: PARTICLE_SIZE / 5 })),
    duration: randomNormal({ mean: SPEED, dev: SPEED * 0.1 }),
    amplitude: randomNormal({ mean: 16, dev: 2.6}),
    offsetY: randomNormal({ mean: -9, dev: 3.5 }),
    arc: Math.PI * 2,
